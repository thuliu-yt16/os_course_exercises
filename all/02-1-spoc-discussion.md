...
- 请描述在“计算机组成原理课”上，同学们做的MIPS CPU是从按复位键开始到可以接收按键输入之间的启动过程。
  ...
  CPU从flash中加载监控程序，将其写入到RAM0x0开始的地址中，完成后PC从0开始执行监控程序。

- x86中BIOS从磁盘读入的第一个扇区是是什么内容？为什么没有直接读入操作系统内核映像？

  启动代码，硬盘分区表，结束标志字。

  因为BIOS在启动硬盘时文件系统还没有建立，硬盘内容无法得知，而且硬盘可以有多个分区，每个分区一个系统，BIOS也不知道启动哪个系统，所以BIOS先读取第一个扇区的内容再把控制权交给MBR。
...
- 以ucore/rcore lab8的answer为例，ucore的系统调用有哪些？大致的功能分类有哪些？
  ...
  ​	进程：exit, fork, wait, exec, yield, kill, getpid,  gettime, set_priority, sleep,

  ​	文件：open, close, read, write, seek, fstat, fsync, getcwd, getdirentry, dup
